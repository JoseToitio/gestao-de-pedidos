...
export const criarPedido = async (novoPedido: { cliente: string; valor: number; descricao: string }) => {
    try {
        const response = await fetch(`${API_URL}/pedidos`, {
            method: 'POST',
...
export const atualizarPedido = async (id: string, dadosAtualizados: { cliente: string; valor: number; descricao: string }) => {
    const response = await fetch(`${API_URL}/pedidos/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
